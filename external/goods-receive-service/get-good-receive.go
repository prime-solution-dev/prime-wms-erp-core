...
func GetGoodsReceives(jsonPayload GoodsReceiveFilter) (GoddsReceiveResult, error) {
	...
	if err != nil {
		return GoddsReceiveResult{}, errors.New("Error reading response body: " + err.Error())
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		if jsonErr := json.Unmarshal(body, &errResp); jsonErr == nil && errResp.Error != "" {
			return GoddsReceiveResult{}, errors.New(errResp.Error)
		}
		return GoddsReceiveResult{}, fmt.Errorf("API error status %d: %s", resp.StatusCode, string(body))
	}

	var dataRes GoddsReceiveResult
	if err = json.Unmarshal(body, &dataRes); err != nil {
		return GoddsReceiveResult{}, errors.New("Error parsing response: " + err.Error())
	}
