...
func GetOrdersDelivery(jsonPayload GetOrderDeliveryRequest) (ResultOrderDeliveryResponse, error) {
	...
	if err != nil {
		return ResultOrderDeliveryResponse{}, errors.New("Error reading response body: " + err.Error())
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		if jsonErr := json.Unmarshal(body, &errResp); jsonErr == nil && errResp.Error != "" {
			return ResultOrderDeliveryResponse{}, errors.New(errResp.Error)
		}
		return ResultOrderDeliveryResponse{}, fmt.Errorf("API error status %d: %s", resp.StatusCode, string(body))
	}

	var dataRes ResultOrderDeliveryResponse
	if err = json.Unmarshal(body, &dataRes); err != nil {
		return ResultOrderDeliveryResponse{}, errors.New("Error parsing response: " + err.Error())
	}
