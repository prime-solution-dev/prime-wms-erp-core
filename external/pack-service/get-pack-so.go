...
func GetPackSo(jsonPayload GetPackingRequest) (ResultPackingResponse, error) {
	...
	if err != nil {
		return ResultPackingResponse{}, errors.New("Error reading response body: " + err.Error())
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		if jsonErr := json.Unmarshal(body, &errResp); jsonErr == nil && errResp.Error != "" {
			return ResultPackingResponse{}, errors.New(errResp.Error)
		}
		return ResultPackingResponse{}, fmt.Errorf("API error status %d: %s", resp.StatusCode, string(body))
	}

	var dataRes ResultPackingResponse
	if err = json.Unmarshal(body, &dataRes); err != nil {
		return ResultPackingResponse{}, errors.New("Error parsing response: " + err.Error())
	}
