...
2. backfill `pdc_percent` / `due_percent` ด้วย `× 100` เฉพาะแถวที่ `percent > 0 AND percent < 1`
3. คำนวณ baht ใหม่เฉพาะแถวที่รายงานระบุว่า "baht เสียหายด้วย" (`pdc` 13 แถว, `due` 15 แถว)
