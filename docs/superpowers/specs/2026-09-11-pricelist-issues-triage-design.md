...
|-------|---------------------|--------|
| 1 ตัวเลขหน้าจอ ≠ เอกสาร | **ไม่เกี่ยว** | สาเหตุคือมาตรฐานหน่วยของ `pdc_percent`/`due_percent` ซึ่งไม่มีใครแตะในรอบก่อน (handoff เดาว่าเป็นเรื่องการปัดเลข `avg_kg_stock` — เดาผิด) |
...
### 1 — ตัวเลขหน้าจอไม่ตรงกับเอกสาร (ไม่แก้ในรอบนี้)

`pdc_percent` / `due_percent` **ไม่มีมาตรฐานหน่วยเดียวในระบบ** มีสองมาตรฐานปนกันอยู่จริงใน DB

| คอลัมน์ | เศษส่วน (<1) | ศูนย์ | จำนวนเต็ม (>=1) |
|---------|-------------|------|----------------|
| `pdc_percent` | 46 แถว (0.01–0.03) | 12 | **5 แถว (1–5)** |
| `due_percent` | 48 แถว (0.015–0.04) | 12 | **3 แถว (2–6)** |

สองมาตรฐานมาจากสองผู้เขียน:

- **ข้างมาก (เศษส่วน)**: `บาท = price × percent` เช่น หมวดI Beams `30.8 × 0.025 = 0.77` ✓
- **ฝั่งหน้าจอ**: `BasePriceTable.vue:419-438` `calculateUpdateTerm` ใช้
  `บาท = percent/100 × price` และแสดงผลเป็น `{{ percent }}%` (บรรทัด 201, 230)
  → คลาดกัน **100×** ทั้งการคำนวณและการแสดงผล

ที่ร้ายกว่านั้น: `calculateUpdatedPrice` (บรรทัด 355-368) วน **ทุก term** แล้วคำนวณทั้ง PDC
และ DUE ใหม่ทุกครั้งที่ผู้ใช้แตะช่อง adjust (`@change`/`@blur` บรรทัด 84-85) แล้ว `.toFixed(2)`
→ แถวที่ percent เก็บเป็นเศษส่วนจะถูกคำนวณเป็น `0.015/100 × 20.5 = 0.003` → ปัดเหลือ **0**
และเซฟทับ DB

พบข้อมูลที่เพี้ยนจากเส้นทางนี้แล้ว 4 แถว (`pdc = 0.01` ทั้งที่ควรเป็น 0.58–0.64):
หมวดเหล็กแผ่น, หมวดเหล็กแบน, หมวดฉาก ราง, หมวดตัวซี ที่ term T3
และ 8 แถวที่ percent ถูกเขียนกลับเป็นจำนวนเต็ม (ตัวซี GI T1/T2/T3, หมวดเหล็กแผ่น T1,
หมวดเหล็กแบน T1 และอีก 3 แถวฝั่ง due)

ฝั่ง export ไม่มีส่วนผิด: `get-price-export-table.go:490-578` `buildBasedPriceTab` เป็น
passthrough ล้วน (`term.Pdc`, `term.PdcPercent`, …) ไม่คำนวณ ไม่ปัด เอกสารจึงสะท้อน DB ตรง ๆ
ยืนยันด้วยไฟล์ export จริงที่ผู้ใช้ส่งมา (sheet `Based price`) ซึ่งตรงกับ DB ทุกค่า

**เหตุที่ไม่แก้**: ทิศทางการแก้ขึ้นกับว่ามาตรฐานไหนเป็นตัวจริง ซึ่งเป็นข้อตกลงทางธุรกิจ
ถ้าเลือกผิดจะทำข้อมูล 94 แถวเสียหาย ส่งมอบเป็นรายงานให้ธุรกิจตัดสินก่อน
(`docs/superpowers/reports/2026-09-11-pricelist-term-percent-convention.sql`)

## แผนแก้: 5 PR แยกตาม issue
...
- **issue 1** — รอธุรกิจตัดสินมาตรฐานหน่วย `pdc_percent`/`due_percent` ส่งมอบเป็นรายงาน
- **backfill ข้อมูลที่เพี้ยนแล้ว** — 8 แถว term ผิดมาตรฐาน, 4 แถว `pdc` ผิด 100×,
